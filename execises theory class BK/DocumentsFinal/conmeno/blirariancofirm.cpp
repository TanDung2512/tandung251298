...
#include "ui_blirariancofirm.h"

blirariancofirm::blirariancofirm(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::blirariancofirm)
{
    ui->setupUi(this);
}

blirariancofirm::~blirariancofirm()
{
    delete ui;
}
#include <QTextStream>
...
void blirariancofirm::on_pushButton_clicked()
{
    Account *acc2;
    acc2 = new Account(acc2->CheckAccount());
    QString code = ui->lineEdit->text();
    QFile Codecheck("System/BlirarianCode.txt");
    ...
    if(Codec == code){
        acc2->Role[1] = "True";
        QMessageBox::warning(this, "Yeah", "U are new Blirarian");
        delete acc2;
        this->close();
    }else{
