    ...
    for(int i=0;i<10;i++){
        if(txtName==Show_Book[i].Book_Name){
           ...
           if( CartFile.open(QIODevice::WriteOnly)){
            ...
            QListWidgetItem item=new QListWidgetItem(QIcon("::/re/icon.png"),Show_Book[i].Book_ID+" "+Show_Book[i].Book_Name);
...
void MainWindow::on_WatchBook_clicked()
{
    QString NameIDBook = ui->listWidget->currentItem()->text();
    QString IDBOOK = NameIDBook.left(4);
    Book *cart;
    cart->load(IDBOOK);
    ui->BookName->setText("Name:"+cart->Book_Name);
    ui->BookAuthor_2->setText("Author: "+cart->Book_Author);
    ui->BookType_2->setText("Type: " + cart->Book_Type);
    ui->BookID_2->setText("Type: " + cart->Book_ID);
    ui->BookRate_2->setText("Rate: " + cart->Rate);
    ui->BookDes_2->setText("Discribe: " + cart->Discribe);
}
